...
def reset_ex2_tab1():
    st.session_state["ex2_prompt_tab1"] = ""
    st.session_state["ex2_response_tab1"] = ""

def reset_ex2_tab2():
    st.session_state["ex2_prompt_tab2"] = ""
    st.session_state["ex2_response_tab2"] = ""
...
st.header("🎓 Exercice 2 – Construire un plan de formation")

tab1, tab2 = st.tabs(["Enoncé", "Conseils"])

with tab1:
    st.markdown(PROMPTFIGHT_CONTENT["ex2"]["enonce"])

    prompt_key = "ex2_prompt_tab1"
    resp_key = "ex2_response_tab1"
    ...
    with col1:
        if st.button("Soumettre la demande au LLM", key="ex2_send_tab1"):
            if not pseudo:
    ...
    with col2:
        st.button("Réinitialiser", key="ex2_reset_tab1", on_click=reset_ex2_tab1)
...
""")
    ...
    with st.expander("📚 Conseils", expanded=False):
        st.markdown(PROMPTFIGHT_CONTENT["ex2"]["conseils"])

    prompt_key = "ex2_prompt_tab2"
    resp_key = "ex2_response_tab2"
    ...
    with col1:
        if st.button("Soumettre la demande au LLM", key="ex2_send_tab2"):
            if not pseudo:
                st.warning("Merci de renseigner un pseudo dans la barre latérale.")
            elif not prompt.strip():
                st.warning("Merci d'écrire un prompt.")
    ...
    with col2:
        st.button("Réinitialiser", key="ex2_reset_tab2", on_click=reset_ex2_tab2)
